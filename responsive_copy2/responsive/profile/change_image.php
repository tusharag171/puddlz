...
$filesa = glob('images_profile/*'); // get all file names
foreach($filesa as $filea){ // iterate files
  if(is_file($filea))
    unlink($filea); // delete file
}

$filesa = glob('images_thumbnail/*'); // get all file names
foreach($filesa as $filea){ // iterate files
  if(is_file($filea))
    unlink($filea); // delete file
}
...
if ($extension == 'gif') {
    imagegif($tmp, $filename,80);
   imagegif($tmp1, $filename1,80);
}
...
/*foreach (glob("1") as $filename) {
    unlink('images_original/'.$filename);
}
*/
...
imagedestroy($tmp1);
  ...
  {
header('Location:incorrect_image.php');
  }


//include("puddlz.com/responsive/profile/profile_page.php?uid=".$_SESSION['uid']);
?>